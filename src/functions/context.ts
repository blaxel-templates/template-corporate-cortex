...
export async function getContext({query}: {query: string}): Promise<string> {
    ...
    try {
        ...
        if (documents.length > 0) {
            ...
            sortedDocs.forEach((doc: { value: string; similarity: number; metadata?: any }) => {
                ...
                docs.forEach((doc) => {
                    ...
                    if (doc.metadata) {
                        ...
                        if (doc.metadata.role || doc.metadata.title) {
                            metadataInfo += ` | Role: ${doc.metadata.role || doc.metadata.title}`;
                        }
                        ...
                        if (doc.metadata.phone || doc.metadata.extension) {
                            metadataInfo += ` | Phone: ${doc.metadata.phone || doc.metadata.extension}`;
                        }
                    }
